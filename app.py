...
def get_info():
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            resp['formats_raw'] = formats # (For debug; optional)
            ...
            if platform == 'youtube':
                ...
                # Only muxed (video+audio) stream, no fallback to mute/highest
                best_muxed = None
                for f in formats:
                    if (f.get('vcodec','none') != 'none'
                        ...
                        and f.get('url')):
                        if (not best_muxed
                            or (f.get('height',0) or 0) > (best_muxed.get('height',0) or 0)
                            or (f.get('tbr',0) or 0) > (best_muxed.get('tbr',0) or 0)):
                            best_muxed = f

                # Audio-only for option
                astreams = [f for f in formats if f.get('acodec','none') != 'none' and f.get('url')]
                best_audio = max(astreams, key=lambda f: (f.get('abr',0) or 0), default=None)
                
                if best_muxed:
                    size_val = best_muxed.get('filesize') or best_muxed.get('filesize_approx') or None
                    resp['video'] = {
                        'resolution': str(best_muxed.get('height', 'HD'))+"p",
                        'format_note': best_muxed.get('format_note') or '',
                ...
                # NO Fallback! Don’t show video if muxed not present.
...
# ======== Cookies API endpoints (optional) ===========
